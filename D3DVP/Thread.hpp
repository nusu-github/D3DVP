...
template <typename T> class WithHolder {
public:
  WithHolder(T &obj) : obj_(obj) { obj_.enter(); }
  ~WithHolder() { obj_.exit(); }

private:
  T &obj_;
};

template <typename T> WithHolder<T> with(T &obj) { return WithHolder<T>(obj); }
...
public:
  ...
  ~CriticalSection() { DeleteCriticalSection(&critical_section_); }
  void enter() { EnterCriticalSection(&critical_section_); }
  void exit() { LeaveCriticalSection(&critical_section_); }
...
public:
  CondWait() : cond_val_(CONDITION_VARIABLE_INIT) {}
  void wait(CriticalSection &cs) {
    SleepConditionVariableCS(&cond_val_, &cs.critical_section_, INFINITE);
  }
  void signal() { WakeConditionVariable(&cond_val_); }
  void broadcast() { WakeAllConditionVariable(&cond_val_); }
...
public:
  ...
  void start() { QueryPerformanceCounter((LARGE_INTEGER *)&prev); }
...
template <typename ErrorHandler> class ThreadBase {
public:
  ErrorHandler *env;
...
public:
  DataPumpThread(size_t maximum, ErrorHandler *env)
      : ThreadBase(env), maximum_(maximum), current_(0), finished_(false) {}
