...
public:
  ...
  virtual ~D3DVP() {
    if (joinCalled == false) {
      MessageBox(NULL,
                 "JoinThreads()が呼ばれる前に派生クラスが終了しました！！",
                 "Error", MB_OK);
    }
#if COUNT_FRAMES
...
TCHAR *track_name[] = {"品質", "幅",   "高さ",
                       "NR",   "EDGE", "調整"}; //	トラックバーの名前
...
FILTER_DLL filter = {
    FILTER_FLAG_EX_INFORMATION |
        FILTER_FLAG_WINDOW_SIZE, //	フィルタのフラグ
                                 ...
                                 //エクスポートメニューを作ります
    320, 400, //	設定ウインドウのサイズ
              //(FILTER_FLAG_WINDOW_SIZEが立っている時に有効)
    ...
    func_update, //	設定が変更されたときに呼ばれる関数へのポインタ
                 //(NULLなら呼ばれません)
    func_WndProc, //	設定ウィンドウにウィンドウメッセージが来た時に呼ばれる関数へのポインタ
                  //(NULLなら呼ばれません)
    ...
    func_save_start, //	セーブが開始される直前に呼ばれる関数へのポインタ
                     //(NULLなら呼ばれません)
    func_save_end, //	セーブが終了した直前に呼ばれる関数へのポインタ
                   //(NULLなら呼ばれません)
};
...
class D3DVPAviUtl {
  ...
  void InitDialog(HWND hwnd) {
    CreateWindow(TEXT("STATIC"), "使用GPU", WS_CHILD | WS_VISIBLE, 10, 320, 300,
